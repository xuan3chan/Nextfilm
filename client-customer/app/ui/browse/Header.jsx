...
export function Header() {
  ...
  return(
    <header className={`transition delay-100 sticky top-0 right-0 left-0 flex items-center justify-between h-[68px] px-[60px] ${roboto.className} ${isScrolled ? 'bg-black' : 'bg-transparent'}`}>
      <div className="flex items-center">
        ...
        <ul className="flex items-center justify-between font-normal text-sm">
          {linkList.map((link, index) => (
            <li key={index} className={`text-white ml-5 ${pathName === link.link ? 'font-bold' : ''}`}>
              <Link href={link.link}>
                {link.name} 
              </Link>
            </li>
